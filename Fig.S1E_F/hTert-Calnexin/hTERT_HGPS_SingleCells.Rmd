---
title: "Single Cell Analysis for hTert HGPS Cells"
...
Recursively search the `data` directory and its subdirectories for files whose name includes the Glob patterns defined in the chunk above, and read the cell-level Columbus data from the results text files.
...
col_tbl <- read_columbus_results("data", glob_path)
...
Calculate number of cells and mean per well for all properties. 4 technical replicates for each of the 3 biological replicates -> 12 wells for marker/cell line combination.
...
```{r Fig.S1_E, fig.width=4,fig.height=4.5, echo = FALSE}
bioreps_tbl %>%
...
```{r Fig.S1_F, fig.width=4,fig.height=4.5, echo = FALSE}
bioreps_defects_tbl %>%
...
Apply the function on a per IF marker and on a per defect category to calculate the p-values on a per
IF marker and defect type basis. 
